...
def do_aug(x,seqlen, y, aug,n_class,class_adaptive=False,multilabel=False):
    policy_y = None
    ...
    mix_fea, _ = aug(x,seqlen, y=policy_y, mode='explore',update_w=False)
    return mix_fea

def hyper_step(model, aug, hyper_params, train_loader, optimizer, val_loader, elementary_lr, neum_steps, device, loss_obj,
        n_class,search_round=4,class_adaptive=False,multilabel=False):
